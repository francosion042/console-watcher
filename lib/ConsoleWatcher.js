...
var temp_1 = __importDefault(require("./modules/temp"));
var SyncLogsToCloud_1 = __importDefault(require("./modules/SyncLogsToCloud"));
var GlobalErrorHandler_1 = __importDefault(require("./errors/GlobalErrorHandler"));
var ConsoleWatcher = /** @class */ (function (_super) {
    __extends(ConsoleWatcher, _super);
    /**
     *
     * @param {object} config
    ...
    function ConsoleWatcher(config) {
        ...
        _this.printInConsole =
            config.printInConsole === undefined ? true : config.printInConsole;
        _this.saveToFile = config.saveToFile === undefined ? true : config.saveToFile;
     ...
     */
    ConsoleWatcher.prototype.syncToConsoleWatcherCloud = function (config) {
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (config.encryptionKey.length !== 16) {
                            return [2 /*return*/, this.handleError(new Error('Invalid encryptionKey length. Expected 16 characters.'))];
                        }
                        logFileType = (0, utils_1.getFileType)(this.logFilePath);
                        logs = [];
                        if (logFileType === 'json') {
                            logs = temp_1.default.json(this.logFilePath);
                        }
                        else {
                            logs = temp_1.default.nonJson(this.logFilePath);
                        }
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, SyncLogsToCloud_1.default.post(encryptedData, config.apiKey, config.applicationKey)];
                    case 2:
                        response = _a.sent();
                        if (response) {
     ...
     */
    ConsoleWatcher.prototype.registerGlobalErrorHandler = function (handler) {
        GlobalErrorHandler_1.default.registerGlobalErrorHandler(handler);
    };
    return ConsoleWatcher;
}(GlobalErrorHandler_1.default));
